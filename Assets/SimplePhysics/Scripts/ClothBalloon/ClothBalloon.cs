...
public class ClothBalloon : MonoBehaviour
{
    [Header("Soft Mesh")]
    ...
    [Header("Pressure")]
    [Tooltip("p = pressureK * (V0 / V). Increase for stronger inflation.")]
    public float pressureK = 2000f;
    public float pMax = 20000f;
    float _V0;
    ...
    private void FixedUpdate()
    {
        ...
        if (_triUnique == null || _triUnique.Length == 0) return;

        float V = Mathf.Abs(ComputeMeshVolumeWorld());
        V = Mathf.Max(V, 1e-6f);

        // balloon-ish pressure: higher when volume is smaller
        float pTarget = pressureK * (_V0 / V);
        pTarget = Mathf.Clamp(pTarget, 0f, pMax);


        float p = pTarget;
        if (p <= 0f) return;

        // apply triangle pressure forces
        for (int t = 0; t < _triUnique.Length; t += 3)
        {
            int i0 = _triUnique[t];
            int i1 = _triUnique[t + 1];
            int i2 = _triUnique[t + 2];

            // degenerate after merge -> skip
            if (i0 == i1 || i1 == i2 || i2 == i0) continue;

            Rigidbody rb0 = _bodies[i0];
            Rigidbody rb1 = _bodies[i1];
            Rigidbody rb2 = _bodies[i2];
            if (!rb0 || !rb1 || !rb2) continue;

            Vector3 v0 = rb0.worldCenterOfMass;
            Vector3 v1 = rb1.worldCenterOfMass;
            Vector3 v2 = rb2.worldCenterOfMass;

            Vector3 cross = Vector3.Cross(v1 - v0, v2 - v0);
            float crossMag = cross.magnitude;
            if (crossMag < 1e-8f) continue;

            float area = 0.5f * crossMag;
            Vector3 n = cross / crossMag; // normalized

            Vector3 F = p * area * n;

            Vector3 per = F / 3f;
            rb0.AddForce(per, ForceMode.Force);
            rb1.AddForce(per, ForceMode.Force);
            rb2.AddForce(per, ForceMode.Force);
        }
    }
    ...
    void BuildSoftMesh()
    {
        ...
        // cache triangles mapped to unique indices
        _triUnique = new int[_triangles.Length];
        for (int i = 0; i < _triangles.Length; i++)
            _triUnique[i] = _originalToUnique[_triangles[i]];
        ...
        // initial volume reference (after bodies exist)
        _V0 = Mathf.Abs(ComputeMeshVolumeWorld());
        _V0 = Mathf.Max(_V0, 1e-6f);
    ...
    float ComputeMeshVolumeWorld()
    {
        // Signed volume via tetrahedra to origin:
        // V = 1/6 * sum dot(v0, cross(v1, v2))
        // Works for closed meshes with consistent winding.
        if (_triUnique == null || _bodies == null) return 0f;
        ...
        for (int t = 0; t < _triUnique.Length; t += 3)
        {
            ...
            sum += Vector3.Dot(v0, Vector3.Cross(v1, v2));
        }
